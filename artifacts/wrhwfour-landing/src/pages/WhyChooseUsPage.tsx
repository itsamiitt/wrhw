    ...
    <PageLayout title="Why Choose Us">
      ...
      <section className="bg-[#0B1520] py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8 relative z-10">
          <motion.div {...fadeUp} className="max-w-3xl">
            <p className="text-primary text-sm font-bold tracking-[0.15em] uppercase mb-4">The WRHWFOUR Difference</p>
      ...
      <section className="py-24 bg-[#FAFAF8]">
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <motion.div {...fadeUp} className="text-center max-w-2xl mx-auto mb-16">
            ...
            <h2 className="text-4xl font-bold text-foreground leading-tight">Six Pillars of Enterprise Trust</h2>
            ...
            {pillars.map((p, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-white border border-gray-100 p-8 rounded shadow-sm hover:shadow-md transition-shadow"
                >
                  <div className="flex items-start gap-6">
                    <div className="shrink-0 w-14 h-14 rounded bg-primary/10 flex items-center justify-center">
                      <Icon className="text-primary" size={24} />
                    </div>
                    <div className="flex-1">
                      <div className="flex items-baseline gap-3 mb-3">
                        <span className="text-3xl font-bold text-primary">{p.stat}</span>
                        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">{p.statLabel}</span>
                      </div>
                      <h3 className="text-xl font-bold text-foreground mb-3">{p.title}</h3>
                      <p className="text-muted-foreground font-light leading-relaxed text-sm mb-4">{p.desc}</p>
                      <ul className="space-y-2">
                        {p.points.map((pt, j) => (
                          <li key={j} className="flex items-start gap-3">
                            <CheckCircle2 className="text-primary shrink-0 mt-0.5" size={16} />
                            <span className="text-sm text-foreground font-light">{pt}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <motion.div {...fadeUp} className="text-center max-w-2xl mx-auto mb-16">
            ...
            <h2 className="text-4xl font-bold text-foreground leading-tight">What Makes Us Different</h2>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {differentiators.map((d, i) => (
              <motion.div
                ...
                className="bg-[#FAFAF8] border border-gray-100 p-7 rounded"
              >
      ...
      <section className="py-24 bg-[#0B1520]">
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <motion.div {...fadeUp} className="text-center max-w-2xl mx-auto mb-16">
            ...
            <h2 className="text-4xl font-bold text-white leading-tight">WRHWFOUR vs. Typical IT Vendors</h2>
          </motion.div>
          ...
          <motion.div {...fadeUp} className="max-w-2xl mx-auto">
            <div className="rounded overflow-hidden border border-white/10">
              <div className="grid grid-cols-3 bg-white/5 px-6 py-4 border-b border-white/10">
                <span className="text-slate-400 text-sm font-semibold uppercase tracking-wider">Criteria</span>
                <span className="text-primary text-sm font-bold uppercase tracking-wider text-center">WRHWFOUR</span>
                <span className="text-slate-500 text-sm font-semibold uppercase tracking-wider text-center">Typical Vendor</span>
              </div>
              {comparisons.map((c, i) => (
                <div
                  key={i}
                  className={`grid grid-cols-3 px-6 py-4 border-b border-white/5 items-center ${i % 2 === 0 ? "bg-white/[0.02]" : ""}`}
                >
                  <span className="text-slate-300 text-sm font-light">{c.criteria}</span>
                  <div className="flex justify-center">
                    <CheckCircle2 className="text-primary" size={20} />
                  </div>
                  <div className="flex justify-center">
                    <span className="w-5 h-5 rounded-full border border-slate-600 flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              >
                ...
                <p className="text-foreground font-light leading-relaxed mb-6 italic">{t.quote}</p>
      ...
      <section className="py-20 bg-white border-t border-gray-100">
        <div className="container mx-auto px-4 md:px-6 lg:px-8 text-center">
          <motion.div {...fadeUp}>
            <h2 className="text-3xl font-bold text-foreground mb-4">Experience the WRHWFOUR Standard</h2>
