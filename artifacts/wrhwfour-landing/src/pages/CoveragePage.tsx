...
export default function CoveragePage() {
  return (
    <PageLayout title="Pan India Coverage">
      ...
      <section className="bg-[#0B1520] py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8 relative z-10">
          <motion.div {...fadeUp} className="max-w-3xl mx-auto text-center">
            <p className="text-primary text-sm font-bold tracking-[0.15em] uppercase mb-4">Nationwide Reach</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((s, i) => {
              ...
              return (
                ...
                >
                  <Icon className="text-white/70 mx-auto mb-2" size={20} />
                  <div className="text-4xl font-bold text-white tracking-tight">{s.value}</div>
                  <div className="text-white/70 text-sm uppercase tracking-widest font-light mt-1">{s.label}</div>
                </motion.div>
              );
            })}
          </div>
      ...
      <section className="py-24 bg-[#FAFAF8]">
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <motion.div {...fadeUp} className="text-center max-w-2xl mx-auto mb-16">
            ...
            <h2 className="text-4xl font-bold text-foreground leading-tight">Coverage by Region</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {zones.map((zone, i) => (
              <motion.div
                ...
                className={`bg-white border ${zone.accent} p-8 rounded shadow-sm`}
              >
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-2xl font-bold text-foreground">{zone.name}</h3>
                  <span className="text-xs font-semibold text-primary border border-primary/30 bg-primary/5 px-3 py-1 rounded-full">{zone.highlight}</span>
                </div>
          ...
          <motion.div {...fadeUp} className="mt-8 bg-[#0B1520] p-6 rounded text-center">
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <motion.div {...fadeUp} className="text-center max-w-2xl mx-auto mb-16">
            ...
            <h2 className="text-4xl font-bold text-foreground leading-tight">Response & Resolution SLAs</h2>
            <p className="text-muted-foreground mt-4 font-light">
              Our service commitments vary by geography and are defined clearly in every contract. No ambiguity, no excuses.
            </p>
          </motion.div>

          <motion.div {...fadeUp} className="max-w-3xl mx-auto overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-[#0B1520]">
                  <th className="px-6 py-4 text-primary text-xs font-bold uppercase tracking-widest">Service Zone</th>
                  <th className="px-6 py-4 text-primary text-xs font-bold uppercase tracking-widest">First Response</th>
                  <th className="px-6 py-4 text-primary text-xs font-bold uppercase tracking-widest">Resolution Target</th>
                  <th className="px-6 py-4 text-primary text-xs font-bold uppercase tracking-widest">Zone Type</th>
                </tr>
              </thead>
              <tbody>
                {slaTable.map((row, i) => (
                  <tr key={i} className={`border-b border-gray-100 ${i % 2 === 0 ? "bg-[#FAFAF8]" : "bg-white"}`}>
                    <td className="px-6 py-4 text-foreground font-medium text-sm">{row.zone}</td>
                    <td className="px-6 py-4 text-primary font-semibold text-sm">{row.response}</td>
                    <td className="px-6 py-4 text-foreground text-sm font-light">{row.resolution}</td>
                    <td className="px-6 py-4">
                      <span className="text-xs font-semibold text-muted-foreground border border-gray-200 px-2 py-1 rounded">{row.type}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <p className="text-center text-muted-foreground text-sm font-light mt-6">
      ...
      <section className="py-24 bg-[#FAFAF8]">
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <motion.div {...fadeUp} className="text-center max-w-2xl mx-auto mb-16">
            ...
            <h2 className="text-4xl font-bold text-foreground leading-tight">How Pan-India Service Actually Works</h2>
          </motion.div>
            ...
            {[
              ...
              return (
                <motion.div
                  ...
                  className="bg-white border border-gray-100 p-8 rounded text-center shadow-sm"
                >
      ...
      {/* CTA */}
      <section className="py-20 bg-[#0B1520]">
        <div className="container mx-auto px-4 md:px-6 lg:px-8 text-center">
          <motion.div {...fadeUp}>
            <h2 className="text-3xl font-bold text-white mb-4">Verify Coverage for Your Location</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <a
                ...
                className="px-8 py-4 border border-white/20 text-white rounded font-medium hover:border-primary hover:text-primary transition-colors inline-flex items-center gap-2 justify-center"
              >
